...
export class JRRagonNetWebServer {
  public readonly onRequestReceived = new ReceivedRequestEmitter();
  private readonly app = express().use(express.json());
  private readonly apiRouters: RouterMap = {};

  public registerApiRouter(name: string) { return this.apiRouters[name] = express.Router(); }

  public startWebServer(): JRRagonNetWebServer {
  ...
  public serveStatic = (staticPath: string) => express.static(path.join(process.cwd(), (process.env.PATH_TO_STATIC_WWW || path.sep), staticPath));
}
